...
extension UICollectionView {
    /**
     空数据显现数据代理对象
     */
    weak public var dzn_Cv_emptyDataSource: AYEmptyDataSource?  {
        get {
            return objc_getAssociatedObject(self, &kEmptyDataSetSource_Cv) as? AYEmptyDataSource
        }
        set(newValue) {
    ...
    /**
     空数据显现代理对象
     */
    weak public var dzn_Cv_emptyDelegate: AYEmptyDelegate? {
        
        get {
            return objc_getAssociatedObject(self, &kEmptyDelegate_Cv) as? AYEmptyDelegate
        }
        set(newValue) {
            if newValue != nil {
                dzn_Cv_invalidate()
...
extension UICollectionView {
    var emptyView_Cv: AYEmptyDataView? {
        get {
            if let view = objc_getAssociatedObject(self, &kEmptyDataSetView_Cv) as? AYEmptyDataView {
                return view
            }
            let view = AYEmptyDataView()
